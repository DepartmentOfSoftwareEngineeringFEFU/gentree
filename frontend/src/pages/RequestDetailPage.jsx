...
export default function RequestDetailPage() {
  ...
  const toggleGeneratedAttachment = (docId) => {
    setCurrentGenerated(prev => {
      ...
      return { ...prev, attached_document_ids: [...selected] }
    })
  }
  ...
  const exportGenerated = async () => {
    ...
    setError('')
    try {
      await saveGeneratedAttachments()
  ...
  const userDocs = docs.filter(doc =>
    doc.archive_request_relation_type === 'USER_ATTACHMENT'
    || (doc.source_type === 'USER_UPLOAD' && doc.document_kind === 'ATTACHMENT')
  )
  const genealogistDocs = docs.filter(doc => !userDocs.includes(doc))
        ...
        {profile ? (
          <>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <p className="muted">
                Статус: {profile.status}. Создан: {profileCreatedAt}.
              </p>
            </div>
          </>
        ) : profileLoading ? (
                ...
                {items.map(d => (
                  <tr key={d.id}>
                    <td>
                      ...
                      <div className="muted" style={{ fontSize: 12, marginTop: 2 }}>{d.mime_type}</div>
                    </td>
                    <td className="muted">{DOC_KIND_LABEL[d.document_kind] ?? d.document_kind}</td>
                    <td>{DOC_SOURCE_LABEL[d.source_type] ?? d.source_type}</td>
            ...
            <div className="col">
              ...
              <label className="col">
                ...
                <textarea
                  ...
                  onBlur={() => api.updateGeneratedComment(currentGenerated.id, {
          ...
          {wizardStep === 'ATTACHMENTS' && currentGenerated && selectedTemplate && (
            <div className="col">
              ...
              {availableAttachments.length === 0 ? (
                ...
                <table>
                  <thead>
                    <tr><th></th><th>Документ</th><th>Комментарий</th><th>Дата загрузки</th><th>Привязка</th></tr>
                  </thead>
                  <tbody>
                    {availableAttachments.map(doc => (
                      <tr key={doc.id}>
                        ...
                        <td className="muted">{doc.comment || '—'}</td>
              ...
              <div className="row">
                ...
                <button className="outline" onClick={() => changeGeneratedStatus('SENT_OUTSIDE_SYSTEM')} disabled={wizardBusy}>Направлено вне системы</button>
              </div>
